...
/**
 * This function is an implementation of the Floyd-Warshall algorithm, which
 * finds the shortest path from each node to every other reachable node in the
 * graph. It is similar to alg.dijkstraAll, but it handles negative edge
 * weights and is more efficient for some types of graphs. This function
 * returns a map of source -> { target -> { distance, predecessor }.
 *
 * The distance property holds the sum of the weights from source to target
 ...
 * edges incident to it for the purposes of shortest path traversal.
 * By default this function uses the graph.outEdges.
